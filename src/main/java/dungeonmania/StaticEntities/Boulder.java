...
public class Boulder extends StaticEntity implements Interactability{
    ...
    public boolean interact(Entity entity, DungeonMap map) {
        ...
        if (entity instanceof Player) {
            ...
            // System.out.println(String.format("Dir %s -> %s", getLocation().toString(), next.toString()));
            if (DungeonMap.isaccessible(map, next, this)) {
                // System.out.println(String.format("Blouder is moved %s -> %s", getLocation(), next));
                if (!entity.getLocation().equals(getLocation()) && DungeonMap.isaccessible(map, getLocation(), entity)) {
                ...
                // else {
                //     // System.out.println(String.format("rtuen back to set %s", player.getPreviousLocation().toString()));
                // System.out.println(String.format("Blouder: get back %s ->  %s", player.getLocation().toString(), player.getPreviousLocation().toString()));
